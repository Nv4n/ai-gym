...
export default async function DashboardPage() {
 ...
 // }
  ...
  // Fetch user's data
  const [ordersResult, reservationsResult, workoutPlansResult] = await Promise.all([
    supabase.from("orders").select("*").eq("user_id", authUser.id).order("created_at", { ascending: false }).limit(5),
    supabase
      ...
      .limit(5),
    supabase.from("workout_plans").select("*").eq("user_id", authUser.id).eq("status", "active").limit(1),
  ])
    ...
    <main className="min-h-screen bg-background">
      <div className="mx-auto max-w-7xl px-4 py-12">
        <div className="mb-12">
          ...
          <p className="text-lg text-muted-foreground">Track your fitness journey and manage your activities</p>
        </div>
            ...
            <CardContent>
              {reservations.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <Button asChild>
                    <Link href="/group-activities">Reserve a Class</Link>
                  </Button>
                </div>
              ) : (
                    ...
                    <div key={reservation.id} className="flex items-center justify-between p-3 rounded-lg border">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {new Date(reservation.reservation_date).toLocaleDateString()} at {reservation.time_slot}
                        </p>
                      </div>
            ...
            <CardContent>
              <div className="grid gap-4 md:grid-cols-3">
                ...
                >
                  ...
                  <p className="font-medium">Reserve a Class</p>
                  <p className="text-sm text-muted-foreground">Book your next group activity</p>
                </Link>
                <Link
                ...
                >
                  ...
                  <p className="font-medium">Generate AI Workout</p>
                ...
                >
                  ...
                  <p className="text-sm text-muted-foreground">Shop supplements and equipment</p>
                </Link>
              </div>
            </CardContent>
          </Card>
