    ...
    <>
      ...
      <main className="min-h-screen bg-background">
        <div className="mx-auto max-w-7xl px-4 py-12">
          <div className="mb-12">
            <h1 className="text-balance text-4xl font-bold mb-4">Group Activities</h1>
            <p className="text-pretty text-lg text-muted-foreground">
              Join our expert-led fitness classes and discover your perfect workout community
            </p>
          </div>

          <div className="grid gap-8 md:grid-cols-3 mb-12">
            {CATEGORIES.map((category) => {
              const classCount = GROUP_ACTIVITIES.filter((c) => c.category === category.id).length
              return (
                <Link key={category.id} href={`/group-activities/${category.id}`}>
                  <Card className="h-full transition-all hover:border-primary hover:shadow-lg cursor-pointer">
                    <CardHeader>
                      <div className="flex items-start justify-between mb-2">
                        <CardTitle className="text-2xl">{category.name}</CardTitle>
                        <Badge variant="secondary">{classCount} classes</Badge>
                      </div>
          ...
          <div className="border-t border-border pt-8">
            <h2 className="text-2xl font-semibold mb-6">All Classes</h2>
