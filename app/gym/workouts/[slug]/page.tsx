    ...
    <>
      ...
      <main className="min-h-screen bg-background">
        <div className="mx-auto max-w-4xl px-4 py-12">
          {/* Header Section */}
          <div className="mb-8">
            <h1 className="text-balance text-4xl font-bold mb-4">{workout.name}</h1>
            <p className="text-pretty text-lg text-muted-foreground mb-6">{workout.description}</p>

            <div className="flex items-center gap-4 mb-6">
              <div className="flex items-center gap-2">
                <Target className="h-5 w-5 text-muted-foreground" />
                <Badge variant={workout.difficulty === "beginner" ? "secondary" : "default"}>
                  {workout.difficulty}
                </Badge>
              </div>
            ...
            <form action={choosePlan}>
              <input type="hidden" name="planId" value={workout.id} />
              <input type="hidden" name="planName" value={workout.name} />
          ...
          <div className="rounded-lg border border-border p-6">
            <h2 className="text-2xl font-semibold mb-6">Full Workout Routine</h2>
              ...
              {workout.exercises.map((exercise, idx) => (
                <div key={idx} className="border-b border-border pb-4 last:border-0 last:pb-0">
                  ...
                  <div className="ml-11 grid grid-cols-2 sm:grid-cols-3 gap-4 mt-2">
                    ...
                    <div>
                      <p className="text-sm text-muted-foreground">Reps/Duration</p>
          ...
          <div className="mt-8 rounded-lg bg-muted p-6">
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              <li>• Warm up with 5-10 minutes of light cardio before starting</li>
              <li>• Focus on proper form over heavy weight</li>
              <li>• Stay hydrated throughout your workout</li>
              <li>• Cool down with stretching after completing all exercises</li>
              <li>{/* Note: Authentication required - will be enforced via Clerk in production */}</li>
            </ul>
          </div>
        </div>
      </main>
